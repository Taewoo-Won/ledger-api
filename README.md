...
`wal.log`에 COMMIT 없는 BEGIN 줄을 하나 심고 재시작했다:

    99|BEGIN|a|b|777|crash-test

부팅 로그:

    [WAL] 미완료 이체 발견: [99] — 직전 실행에서 BEGIN 후 COMMIT을 못 남긴 연산이다

정상 완료된 seq 1은 걸리지 않고 COMMIT이 없는 seq 99만 잡힌다.
서버는 재시작 시 백지에서 출발하지 않고, 직전 실행의 흔적을 확인하고 시작한다.

### seq 연속성 확인 (실측)

재시작 후 새 이체 1건. 로그의 마지막 두 줄:

    2|BEGIN|a|b|300|seq-2
    2|COMMIT|a|b|300|seq-2

seq가 1부터 다시 시작하지 않고 기존 로그의 최대값을 이어받는다.
번호가 겹치면 같은 seq의 BEGIN이 둘이 되어 미완료 판별이 무너지므로,
WalWriter 생성자가 부팅 시 로그를 훑어 최대 seq에서 이어 센다.
